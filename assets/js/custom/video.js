...
function toggleTestimonialVideo(event) {
  event.stopPropagation(); // Prevents additional events
  ...
  const replacementImg = container.querySelector(".testimonial-video-img");
  const playButton = container.querySelector(".play-btn");
...
function toggleCaseVideo(event) {
  ...
  // Find the video and replacement image inside the clicked container
  const video = container.querySelector(".case-study-video");
  const replacementImg = container.querySelector(".case-video-img");
  const playButton = container.querySelector(".play-btn");
